...
namespace W3WGame.Ka.Controller.code
{
    public class codeController:BaseController
    {
        ...
        {
            var list = _gameKaTask.GetKaPagedList(title, (int)KaTypeEnum.JiHuo, pageindex, pagesize);
            var model = new PagedList<NewsKaDto>(list.ToList(), pageindex, pagesize, list.TotalItemCount);
            ViewData["randlist"] = _gameKaTask.GetRanKa((int)KaTypeEnum.JiHuo, 10);
            return View(model);
        }
    }
}
