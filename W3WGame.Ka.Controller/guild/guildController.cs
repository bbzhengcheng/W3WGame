...
namespace W3WGame.Ka.Controller.guild
{
    public class guildController:BaseController
    {
        ...
        {
            var list = _gameKaTask.GetKaPagedList(title, (int)KaTypeEnum.GuildCard, pageindex, pagesize);
            var model = new PagedList<NewsKaDto>(list.ToList(), pageindex, pagesize, list.TotalItemCount);
            ViewData["randlist"] = _gameKaTask.GetRanKa((int)KaTypeEnum.GuildCard, 10);
            return View(model);
        }
    }
}
